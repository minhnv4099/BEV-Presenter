...
class LoggerHook(LoggerHook):
    ...
    def after_train_iter(self,
        ...
        if self.log_exact_outputs:
            pattern = r"^.*?data_time:\s*\d+\.\d+"

            prefix_log = re.search(pattern, log_str)
            if prefix_log:
                prefix_log = prefix_log.group()

            for key, value in outputs.items():
                output_str = f'{key}: {value:.{4}f}'
                prefix_log = prefix_log + ' ' + output_str
