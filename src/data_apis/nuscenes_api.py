...
class CustomNuScenes:
    ...
    def __init__(self,
        ...
        self.table_names = [
            ...
            'ego_pose', 'sensor', 'calibrated_sensor',
        ...
        if verbose:
            logger.info("========== Loading NuScenes tables for version {}...".format(self.version))
        ...
        self.sample_data: list[SampleDataRecord] = self.__load_table__('sample_data')
    ...
    def __make_reverse_index__(self, verbose: bool, save: bool = False, save_dir: str = 'reverse') -> None:
        ...
        start_time = time.time()
        if verbose:
            logger.info("Reverse indexing ...")

        # Store the mapping from token to table index for each table.
        self._token2ind = dict()
        self._ind2token = dict()
        for table in self.table_names:
            self._token2ind[table] = dict()
            self._ind2token[table] = dict()

            for ind, member in enumerate(getattr(self, table)):
                self._token2ind[table][member['token']] = ind
                self._token2ind[table][ind] = member['token']

        # Reverse-index samples with sample_data and annotations.
        # get associated data and annotation for each snapshot
        ...
        for sd_record in self.sample_data:
            ...
            sensor_record = self.get('sensor', cs_record['sensor_token'])
            # add modality and channel to sample data
    ...
    def field2token(self, table_name: str, field: str, query) -> List[str]:
        ...
        Returns:
            ist of tokens for the matching records.
        """
        matches = []
    ...
    def get_sample_data(self, sample_data_token: str,
        ...
        Args:
            ...
            use_flat_vehicle_coordinates:
                Instead of the current sensor's coordinate frame,
