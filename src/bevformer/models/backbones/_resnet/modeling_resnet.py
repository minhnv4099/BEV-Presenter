...
from src.utils.docstring import auto_docstring
...
class ResNetConvLayer(nn.Module):
    """The Layer in ResNet architecture with 1 convolutional layer followed by 1 bath norm layer,
    and the last one is an activation

    Args:
        in_channels (int): Input channel for [`conv`] layer.
        out_channels (input): Output channel for [`conv`] layer.
        kernel_size (int, Optional): Size of square kernel. Default to 3
        stride (int): Number of striding steps. Default to 1.
        activation (str): Name of activation function. Default to `relu`.

    example::

        Conv2d -> BathNorm2d -> activation
    """
    def __init__(
        self,
        in_channels: int,
...
class ResNetBottleNeckLayer(nn.Module):
    """
    A classic ResNet's bottleneck layer composed by three convolutions (1x1, 3x3, 1x1).

    The first `1x1` convolution reduces the input by a factor of `reduction` in order to make the second `3x3`
    convolution faster. The last `1x1` convolution resample the reduced features to `out_channels`. If
...
class ResNetEncoder(nn.Module):
    ...
    ) -> BaseModelOutputWithNoAttention | tuple:
        # include the first hidden state
...
class ResNetPreTrainedModel(BaseModule):
    config: ResNetConfig
...
class ResNet(ResNetPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        feature_maps = ()
        for idx in self.config.out_indices:
