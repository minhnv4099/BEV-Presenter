...
class FocalLoss(nn.Module):
    ...
    def __init__(self,
                 ...
                 gamma: float = 2.0,
                 alpha: float = 0.25,
                 reduction: Literal[None, "none", "mean", "sum"] = 'mean',
        ...
        assert use_sigmoid is True, 'Now only support focal loss sigmoid.'
    ...
    def forward(self,
                ...
                reduction_override: Literal[None, "none", "mean", "sum"] = None
                ) -> torch.Tensor:
        """Forward function.
        ...
        if self.use_sigmoid:
            if self.activated:
                ...
                calculate_loss_func: Callable[..., torch.Tensor]
                if pred.dim() == target.dim():
