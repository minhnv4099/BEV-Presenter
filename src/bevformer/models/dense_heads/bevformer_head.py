...
from ...models.utils.misc import multi_apply
...
class BEVFormerHead(DETRHead):
    ...
    Args:
        ...
        positional_encoding (:obj:`ConfigDict` or dict): Config of the positional encoding. Default to `None`.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.code_weights = kwargs.get('code_weights',
        ...
        super().__init__(**kwargs, transformer=transformer)

        self.positional_encoding = build_positional_encoding(positional_encoding)
        """Position encoding of bev plane."""
    ...
    def _init_layers(self):
        ...
        # encoder feature map when as_two_stage is True.
        ...
        if self.with_box_refine:
            ...
            # identical classifier and regressor
            self.cls_branches = nn.ModuleList([fc_cls for _ in range(num_pred)])
            self.reg_branches = nn.ModuleList([reg_branch for _ in range(num_pred)])

        if not self.as_two_stage:
            self.bev_embedding = nn.Embedding(self.bev_h * self.bev_w, self.embed_dims)
            # embed_dims * 2 means having query and position
    ...
    def forward(
        ...
        """Forward function.
        Args:
            mlvl_feats (list[Tensor]): List of `n_level` features from the neck
                network, each is a 5D-tensor with shape `(bs, n_cam, c, h, w)`.
            prev_bev (Tensor): Previous bev features.
                Shape of `(bs, n_query, embed_dims)`.
            only_bev (bool): Only compute BEV features with encoder.
            img_metas (list[dict]): Image metadata list of `bs` of dict (cam).
        Returns:
            dict: Dict with keys:
        ...
        # (n_dec_layer, bs, n_query, emb_dims)
    ...
    def loss_single(self,
        ...
        if self.sync_cls_avg_factor:
            cls_avg_factor = torch.mean(   # NOTE: reduce_mean
...
@HEADS.register_module()
class BEVFormerHeadGroupDETR(BEVFormerHead):
    def __init__(self,
                 *args,
                 group_detr=1,
                 **kwargs):
