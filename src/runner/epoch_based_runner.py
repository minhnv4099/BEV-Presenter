...
from src.registry import RUNNERS
...
class EpochBasedRunner(BaseRunner):
    ...
    @torch.no_grad()
    def val(self, data_loader: DataLoader, **kwargs):
        self.model.eval()
        self.mode = 'val'
        self.data_loader = data_loader
...
@RUNNERS.register_module()
class EpochBasedRunnerV2(EpochBasedRunner):
    """
    # basic logic
    ...
    prev_bev = None
    for each in input_sequence[:-1]
        prev_bev = eval_model(each, prev_bev)) # inference only.

    model(input_sequence[-1], prev_bev) # train the last sample.
    """
